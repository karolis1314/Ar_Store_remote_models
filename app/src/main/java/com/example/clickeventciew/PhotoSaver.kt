...
class PhotoSaver(
    ...
    private fun generateFilename(): String? {
        val date = SimpleDateFormat("yyyyMMddHmmss", Locale.getDefault()).format(Date())
        return Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM)?.absolutePath +
                "/arStore/${date}_screenshots.jpg"
    }
    ...
    private fun saveBitmapToGallery(bmp: Bitmap) {
        ...
        val contentValues = ContentValues().apply {
            ...
            put(MediaStore.MediaColumns.RELATIVE_PATH, "DCIM/ArStore")
        }
        ...
        activity.contentResolver.openOutputStream(uri ?: return).use { outputStream ->
            outputStream?.let {
                try {
                    ...
                    Toast.makeText(activity, "Failed to save bitmap", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
